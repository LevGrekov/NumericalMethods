...
class SquareRootMethodSLAU(val A: SqComplexMatrix, val b: ComplexMatrix) {
    ...
    private fun findS(i:Int, j:Int): ComplexNum = when {
        i == 0 && j == 0 -> ComplexNum(sqrt(A[0,0].re/d[0,0].re))
        i == 0 && j != 0 -> (A[0,j]/(findS(0,0)*d[0,0].re))
        i == j -> ComplexNum(sqrt(abs(A[i, i].re - (0 until i).sumOf { k -> s[k,i].abs2() * d[k,k].re })),0.0)
        i < j -> {
